...
cmd_eval = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/evaluation/eval_cls_cifar10.py --dataset {dataset} --testpath {testpath} --metric {metric} --savingpath {savingpath} \n'
...
with open('./results/cifar/cifar10_estim.txt', 'w') as fpr:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./results/cifar/cifar10_estim.txt')

# evaluate 36 condtions for all test conditions
with open('./results/cifar/cifar10_eval.txt', 'w+') as fpr:
    for metric in metrics:
        for training_cond in training_conds:
            ext = training_cond.split('/')[-1]
            if ext == 'baseline':
...
print(f'fsl_sub -q long -R 128 -l logs -t ./results/cifar/cifar10_eval.txt')