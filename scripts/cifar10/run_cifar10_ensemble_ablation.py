...
cmd_estim = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/estimation/estim_cls_ensemble_ablation.py --dataset {dataset} --numcls {numcls} --metric {metric} --valpath {valpath} --portion {portion}\n'
cmd_eval = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/evaluation/eval_cls_cifar10_ensemble_ablation.py --dataset {dataset} --testpath {testpath} --metric {metric} --savingpath {savingpath} --portion {portion}\n'
...
with open('./cifar10_estim_ensemble_ablation.txt', 'w') as fpr:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./cifar10_estim_ensemble_ablation.txt')

# evaluate 36 condtions for all test conditions
with open('./cifar10_eval_ensemble_ablation.txt', 'w+') as fpr:
    for metric in metrics:
        for training_cond in training_conds:
            for portion in portions:
                ext = training_cond.split('/')[-1]
                ...
                for test_syn_cond in test_syn_conds:
                    ...
                    savingpath = f"./results_{dataset}_{metric}_{test_syn_cond}_ensemble_{portion}.txt"
                    fpr.write(cmd_eval.format(dataset=dataset, testpath=f'"{testpath}"', metric=metric, savingpath=f'"{savingpath}"', portion=portion))

print(f'fsl_sub -q long -R 128 -l logs -t ./cifar10_eval_ensemble_ablation.txt')