...
cmd_estim = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/estimation/estim_cls.py --dataset {dataset} --numcls {numcls} --metric {metric} --valpath {valpath} --batch {batch} \n'
cmd_eval = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/evaluation/eval_cls_cifar100.py --dataset {dataset} --testpath {testpath} --metric {metric} --savingpath {savingpath} \n'
...
with open('./results/cifar/cifar100_estim.txt', 'w') as fpr:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./results/cifar/cifar100_estim.txt')

# evaluate 36 condtions for all test conditions
with open('./results/cifar/cifar100_eval.txt', 'w') as fpr:
    for metric in metrics:
        for training_cond in training_conds:
            ...
            for test_syn_cond in test_syn_conds:
                ...
                savingpath = f"./results/cifar/results_{dataset}_{metric}_{test_syn_cond}.txt"
                fpr.write(cmd_eval.format(dataset=dataset, testpath=f'"{testpath}"', metric=metric, savingpath=f'"{savingpath}"'))

print(f'fsl_sub -q long -R 128 -l logs -t ./results/cifar/cifar100_eval.txt')
