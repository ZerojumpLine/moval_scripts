...
cmd_estim = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/estimation/estim_seg3d.py --dataset {dataset} --predpath {predpath} --metric {metric} --gtpath {gtpath} --partion {partion} \n'
cmd_eval = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/evaluation/eval_seg3d_brainlesion.py --dataset {dataset} --predpath {predpath} --gtpath {gtpath} --metric {metric} --savingpath {savingpath} \n'
...
with open('./results/brainlesion/brainlesion_estim_dsc.txt', 'w') as fpr:
    metric = metrics[0]
...
print(f'fsl_sub -q long -R 128 -l logs -t ./results/brainlesion/brainlesion_estim_dsc.txt')

# estimate 36 conditions for 4 other metrics
with open('./results/brainlesion/brainlesion_estim_4metrics.txt', 'w') as fpr:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./results/brainlesion/brainlesion_estim_4metrics.txt')

# evaluate 36 condtions for all test conditions
with open('./results/brainlesion/brainlesion_eval_dsc.txt', 'w') as fpr:
    metric = metrics[0]
    for training_cond in training_conds:
        ...
        for test_syn_cond in test_syn_conds:
            ...
            gtpath = f"{datapath}/Siemens Trio"
            savingpath = f"./results/brainlesion/results_{dataset}_{metric}_syn_{test_syn_cond}.txt"
        ...
        for test_nat_cond in test_nat_conds:
            ...
            savingpath = f"./results/brainlesion/results_{dataset}_{metric}_nat_{test_nat_cond}.txt"
            fpr.write(cmd_eval.format(dataset=dataset, predpath=f'"{predpath}"', gtpath=f'"{gtpath}"', metric=metric, savingpath=f'"{savingpath}"'))

print(f'fsl_sub -q long -R 128 -l logs -t ./results/brainlesion/brainlesion_eval_dsc.txt')

# evaluate 36 condtions for 4 other metrics
with open('./results/brainlesion/brainlesion_eval_4metrics.txt', 'w') as fpr:
    training_cond = training_conds[0]
    for metric in metrics[1:]:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./results/brainlesion/brainlesion_eval_4metrics.txt')
