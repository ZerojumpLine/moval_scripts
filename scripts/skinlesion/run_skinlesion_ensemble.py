...
cmd_estim = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/estimation/estim_cls_ensemble.py --dataset {dataset} --numcls {numcls} --metric {metric} --valpath {valpath} \n'
cmd_eval = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/evaluation/eval_cls_skinlesion_ensemble.py --dataset {dataset} --testpath {testpath} --metric {metric} --savingpath {savingpath} \n'
...
with open('./skinlesion_estim_ensemble.txt', 'w') as fpr:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./skinlesion_estim_ensemble.txt')

# evaluate 36 condtions for all test conditions
with open('./skinlesion_eval_ensemble.txt', 'w') as fpr:
    for metric in metrics:
        for training_cond in training_conds:
            ...
            for test_syn_cond in test_syn_conds:
                ...
                for knum in range(1, 6):
                    ...
                    savingpath = f"./results_{dataset}_{metric}_{test_syn_cond}_{knum}_ensemble.txt"
            ...
            for testnat_cond in test_nat_conds:
                ...
                savingpath = f"./results_{dataset}_{metric}_{testnat_cond}_ensemble.txt"
                fpr.write(cmd_eval.format(dataset=dataset, testpath=f'"{testpath}"', metric=metric, savingpath=f'"{savingpath}"'))

print(f'fsl_sub -q long -R 128 -l logs -t ./skinlesion_eval_ensemble.txt')