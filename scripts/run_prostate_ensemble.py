...
cmd_estim = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/estimation/estim_seg3d_ensemble.py --dataset {dataset} --predpath {predpath} --metric {metric} --gtpath {gtpath} \n'
cmd_eval = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/evaluation/eval_seg3d_prostate_ensemble.py --dataset {dataset} --predpath {predpath} --gtpath {gtpath} --metric {metric} --savingpath {savingpath} \n'
...
with open('./prostate_estim_ensemble.txt', 'w') as fpr:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./prostate_estim_ensemble.txt')

# evaluate 36 condtions for all test conditions
with open('./prostate_eval_ensemble.txt', 'w') as fpr:
    for metric in metrics:
        for training_cond in training_conds:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./prostate_eval_ensemble.txt')