...
cmd_estim = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/estimation/estim_seg3d.py --dataset {dataset} --predpath {predpath} --metric {metric} --gtpath {gtpath} \n'
cmd_eval = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/src/evaluation/eval_seg3d_prostate.py --dataset {dataset} --predpath {predpath} --gtpath {gtpath} --metric {metric} --savingpath {savingpath} \n'
...
with open('./results/prostate/prostate_estim.txt', 'w') as fpr:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./results/prostate/prostate_estim.txt')

# evaluate 36 condtions for all test conditions
with open('./results/prostate/prostate_eval.txt', 'w') as fpr:
    for metric in metrics:
        for training_cond in training_conds:
            ...
            for test_syn_cond in test_syn_conds:
                ...
                gtpath = f"{datapath}/BMC"
                savingpath = f"./results/prostate/results_{dataset}_{metric}_syn_{test_syn_cond}.txt"
            ...
            for test_nat_cond in test_nat_conds:
                ...
                savingpath = f"./results/prostate/results_{dataset}_{metric}_nat_{test_nat_cond}.txt"
                fpr.write(cmd_eval.format(dataset=dataset, predpath=f'"{predpath}"', gtpath=f'"{gtpath}"', metric=metric, savingpath=f'"{savingpath}"'))

print(f'fsl_sub -q long -R 128 -l logs -t ./results/prostate/prostate_eval.txt')