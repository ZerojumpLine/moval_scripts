...
cmd_estim = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/estim_seg2d_ensemble.py --dataset {dataset} --predpath {predpath} --metric {metric} --gtpath {gtpath} \n'
cmd_eval = bin + ' /well/win-fmrib-analysis/users/gqu790/moval/moval_scripts/eval_seg2d_cardiac_ensemble.py --dataset {dataset} --predpath {predpath} --gtpath {gtpath} --metric {metric} --savingpath {savingpath} \n'
...
with open('./cardiac_estim_ensemble.txt', 'w') as fpr:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./cardiac_estim_ensemble.txt')

# evaluate 36 condtions for all test conditions
with open('./cardiac_eval_ensemble.txt', 'w') as fpr:
    for metric in metrics:
        for training_cond in training_conds:
...
print(f'fsl_sub -q long -R 128 -l logs -t ./cardiac_eval_ensemble.txt')